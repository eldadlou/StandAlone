...
{
    /// <summary>
    /// Simple automatic team assignment and unit spawning for testing
    /// Spawns units for each team automatically with good separation
    /// </summary>
    public class TeamAssignmentTest : MonoBehaviour
    {
        ...
        [Header("Spawn Center")]
        [SerializeField] private Transform spawnCenter; // Optional: set a specific spawn center
        ...
        public void SpawnAllUnits()
        {
            ...
            Debug.Log($"🎮 TeamAssignmentTest: Spawn center: {center}");
            
            // Spawn player units (left side)
            Vector3 playerCenter = center + Vector3.left * teamSeparation;
            Debug.Log($"🎮 TeamAssignmentTest: Player center: {playerCenter}");
            SpawnTeamUnits(playerUnitTypes, playerTeam, playerCenter);
            
            // Spawn AI units (right side)
            Vector3 aiCenter = center + Vector3.right * teamSeparation;
            Debug.Log($"🎮 TeamAssignmentTest: AI center: {aiCenter}");
        ...
        private void SpawnTeamUnits(UnitType[] unitTypes, Team team, Vector3 teamCenter)
        {
            ...
            for (int i = 0; i < unitTypes.Length; i++)
            {
                ...
                Vector3 spawnPosition = teamCenter + Vector3.forward * (i * unitSpacing);
                Debug.Log($"🎮 TeamAssignmentTest: Attempting to spawn {unitTypes[i]} at {spawnPosition}");
                
                // Find ground position
                spawnPosition = FindGroundPosition(spawnPosition);
                Debug.Log($"🎮 TeamAssignmentTest: Ground position found: {spawnPosition}");
                
                // Create unit using pool system
        ...
        {
            Vector3 raycastStart = position + Vector3.up * 100f;
            ...
            // Fallback: use original position at ground level
            position.y = 0;
            return position;
        }
