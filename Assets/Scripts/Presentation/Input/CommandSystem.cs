...
namespace MyGame.Input
{
    ...
    public class CommandSystem : MonoBehaviour
    {
        ...
        private void HandleMoveCommand(IUnit unit, Vector3 destination)
        {
            ...
            var targetPosition = destination;
            if (useNavMeshValidation && pathfindingSystem != null)
                targetPosition = pathfindingSystem.GetNearestValidPosition(targetPosition, 50f);
        ...
        private void HandleUnitCommand(Vector2 screenPosition)
        {
            ...
            if (!SelectionUtility.TryGetMoveTargetAtScreen(screenPosition, out var targetPosition))
                return;

            if (useNavMeshValidation && pathfindingSystem != null)
                targetPosition = pathfindingSystem.GetNearestValidPosition(targetPosition, 50f);

            var distance = Vector3.Distance(units[0].Position, targetPosition);
            if (distance > maxCommandDistance)
            {
                Debug.LogWarning($"Target too far: {distance:F1}m (max: {maxCommandDistance}m)");
                return;
        ...
        private void HandleUnitSelection(Vector2 screenPosition)
        {
            ...
            if (SelectionUtility.TryGetSelectableAtScreen(screenPosition, out var selectable))
            {
                ...
                if (Debug.isDebugBuild)
                    Debug.Log($"Single unit selected: {selectable.Name}");
                return;
            }
        ...
        private void MoveUnitsInFormation(System.Collections.Generic.List<ISelectableUnit> units, Vector3 targetPosition)
        {
            ...
            if (useNavMeshValidation && pathfindingSystem != null)
            {
                ...
                {
                    formationPositions[i] = pathfindingSystem.GetNearestValidPosition(formationPositions[i], 50f);
                }
            }
        ...
        private void MoveUnitsIndividually(System.Collections.Generic.List<ISelectableUnit> units, Vector3 targetPosition)
        {
